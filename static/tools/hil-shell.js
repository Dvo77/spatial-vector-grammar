...
(function () {
  ...
  // ─── FIREBASE CDN URLS (modular SDK v10) ────────────────────────────────────
  ...
  // ─── CSS DESIGN TOKENS + SHELL STYLES ───────────────────────────────────────
  const SHELL_CSS = `
    :root {
      --hil-green:       #00cc66;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hil-auth-divider::before,
    .hil-auth-divider::after {
      content: '';
      flex: 1;
      height: 1px;
      background: var(--hil-border);
    }
  ...
  const LOGO_SVG = `
    <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" id="hil-logo">
      ...
      <text x="50" y="54" text-anchor="middle"
            font-family="Orbitron,monospace" font-size="7"
            fill="#00cc66" letter-spacing="1">HIL</text>
    </svg>
  `;
  ...
  // ─── EMAIL ICON SVG ──────────────────────────────────────────────────────────
  const EMAIL_ICON = `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="2">
  ...
  function buildHeader() {
    ...
    document.getElementById('hil-signout-btn').addEventListener('click', () => {
      if (firebaseAuth) {
        ...
        HILShell.toast('Signed out', 'info');
      }
    });
  }
    ...
    gate.innerHTML = `
      <div id="hil-auth-box">
        ...
        <div class="hil-auth-panel active" id="hil-panel-social">
          <button class="hil-social-btn" id="hil-google-btn">
            ${GOOGLE_ICON} Continue with Google
          </button>
          <button class="hil-social-btn" id="hil-github-btn">
  ...
  function buildToastContainer() {
    ...
    container.id = 'hil-toast-container';
    document.body.appendChild(container);
  }

  // ─── USER RECORD INIT (Firestore v1.3 schema) ────────────────────────────────
  ...
  async function onAuthSuccess(user, setDoc, doc) {
    ...
    HILShell.toast(`Welcome back, ${user.displayName || user.email}`, 'success');

    // Call tool's onAuth callback
  ...
  function wireAuth(GoogleAuthProvider, GithubAuthProvider, signInWithPopup,
    ...
    document.getElementById('hil-email-submit').addEventListener('click', async () => {
      ...
      const password = document.getElementById('hil-password-input').value;
    ...
    document.getElementById('hil-reg-submit').addEventListener('click', async () => {
      ...
      const password = document.getElementById('hil-reg-password-input').value;
      if (!email || !password) { showAuthError('Email and password required.'); return; }
  ...
  async function loadFirebaseAndBoot() {
    // Load Firebase modules via dynamic import
    const [
    ...
    onAuthStateChanged(firebaseAuth, async (user) => {
      if (user) {
        ...
        // Signed out — show gate if auth required
        window.currentUser = null;
        const gate = document.getElementById('hil-auth-gate');
        if (gate && shellConfig.requireAuth !== false) {
  ...
  const HILShell = {
    ...
    init(config) {
      ...
      // Pre-register window functions as no-ops to prevent setMode-class errors
    ...
    toast(message, type = 'info', duration = 3000) {
      ...
      toast.className = `hil-toast ${type === 'error' ? 'error' : type === 'warning' ? 'warning' : ''}`;
